...
class Matchdays(LoginRequiredMixin, ListView):
	...
	def get_context_data(self, **kwargs):
		...
		LAS_list = LineupAnswerSet.objects.filter(for_question__for_matchday__day = pk)
		GRAS_list = GroupRanksAnswerSet.objects.filter(for_question__for_matchday__day = pk)
		TAVAS_list = TeamAndValueAnswerSet.objects.filter(for_question__for_matchday__day = pk)
		GAAAS_list = GoalAndAssistAnswerSet.objects.filter(for_question__for_matchday__day = pk)
		aset_list = list(chain(SPTAS_list, YNAS_list, WDLAS_list, SAS_list, SIAS_list, LAS_list, GRAS_list, TAVAS_list, GAAAS_list))
		...
		LA_list = LineupAnswer.objects.filter(by_player = user, for_question__for_matchday__day = pk)
		GRA_list = GroupRanksAnswer.objects.filter(by_player = user, for_question__for_matchday__day = pk)
		TAVA_list = TeamAndValueAnswer.objects.filter(by_player = user, for_question__for_matchday__day = pk)
		GAAA_list = GoalAndAssistAnswer.objects.filter(by_player = user, for_question__for_matchday__day = pk)
		a_list = list(chain(SPTA_list, YNA_list, WDLA_list, SA_list, SIA_list, LA_list, GRA_list, TAVA_list, GAAA_list))
...
class SubmitAnswer(LoginRequiredMixin, CreateView):
	...
	def get_form_class(self, **kwargs):
		...
		LA_list = LineupAnswer.objects.filter(by_player = user, for_question__for_matchday = question.for_matchday)
		GRA_list = GroupRanksAnswer.objects.filter(by_player = user, for_question__for_matchday = question.for_matchday)
		TAVA_list = TeamAndValueAnswer.objects.filter(by_player = user, for_question__for_matchday = question.for_matchday)
		GAAA_list = GoalAndAssistAnswer.objects.filter(by_player = user, for_question__for_matchday = question.for_matchday)
		a_list = list(chain(SPTA_list, YNA_list, WDLA_list, SA_list, SIA_list, LA_list, GRA_list, TAVA_list, GAAA_list))
...
class MatchdayPerformance(LoginRequiredMixin, ListView):
	...
	def get_context_data(self, **kwargs):
		...
		LA_list = LineupAnswer.objects.filter(for_question__for_matchday__day = pk)
		GRA_list = GroupRanksAnswer.objects.filter(for_question__for_matchday__day = pk)
		TAVA_list = TeamAndValueAnswer.objects.filter(for_question__for_matchday__day = pk)
		GAAA_list = GoalAndAssistAnswer.objects.filter(for_question__for_matchday__day = pk)
		a_list = list(chain(SPTA_list, YNA_list, WDLA_list, SA_list, SIA_list, LA_list, GRA_list, TAVA_list, GAAA_list))
...
class LeagueTable(LoginRequiredMixin, ListView):
	...
	def get_context_data(self, **kwargs):
		...
		LA_list = LineupAnswer.objects.all()
		GRA_list = GroupRanksAnswer.objects.all()
		TAVA_list = TeamAndValueAnswer.objects.all()
		GAAA_list = GoalAndAssistAnswer.objects.all()
		a_list = list(chain(SPTA_list, YNA_list, WDLA_list, SA_list, SIA_list, LA_list, GRA_list, TAVA_list, GAAA_list))
